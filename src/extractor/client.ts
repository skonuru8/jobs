...
export async function complete(opts: CompletionOptions): Promise<CompletionResult> {
  ...
  // Strict json_schema is preferred — eliminates markdown-fence and
  // missing-field edge cases entirely. Set EXTRACTOR_FORCE_JSON_OBJECT=1
  // to fall back if the model rejects strict mode.
  const useStrict = !process.env.EXTRACTOR_FORCE_JSON_OBJECT;
